...
> 피부 사진 업로드 → 5종 피부 상태 분류 → 아토피 예측 시 건강설문 → 스크리닝 결과 및 종합 보고서 제공
...
- **아토피 가능성 스크리닝** — 건강·생활습관 정보를 기반으로 아토피 진단군에서 관찰된 패턴 확인
...
AI Hub 합성 이미지에서는 99.60%의 정확도를 보였지만 외부 실사 이미지 25장에서는 52%까지 하락했습니다. 오분류 사례를 확인한 결과 조명·배경·피부톤이 표준화된 학습 데이터와 실제 촬영 환경의 차이가 주요 원인으로 판단됐습니다.

DermNet 아토피 데이터를 추가 정제해 재학습하는 실험도 진행했지만 실사 정확도가 52% → 44%로 오히려 감소해 해당 버전은 채택하지 않았습니다.
...
초기 앱에서는 0.10·0.25를 기준으로 낮음/보통/높음 3단계를 제공했지만 근거가 명확히 남아 있지 않았습니다.

Calibration holdout에서 threshold를 다시 검토해 Youden's J 기준 0.11을 운영점으로 채택하고, 근거 없는 3단계 등급 대신 기준치 이상/미만의 이진 스크리닝으로 변경했습니다.
