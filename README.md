...
**개발 기간** 2026.04 (약 1개월) · **팀 구성** 4명 (기획 1 · 데이터분석 1 · 기술 2)

**담당 역할** 기술 파트 2명 중 1명으로 이미지 분류 딥러닝 모델링과 Streamlit 웹 서비스(로그인, 4개 페이지, DB 설계) 전체 구현을 담당했습니다. 문진 위험도 모델은 팀 내 데이터분석 파트에서 KNHANES 데이터를 전처리·모델링한 결과를 넘겨받아 웹 서비스에 연동했습니다.
