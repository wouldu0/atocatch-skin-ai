...
> 피부 사진 업로드 + 생활습관 문진 → 5종 피부 질환 분류 & 아토피 가능성 스크리닝 → 맞춤형 보고서 제공
...
2. **문진형 아토피 가능성 스크리닝 모델** — KNHANES 건강설문 데이터를 활용해 아토피 의사진단 이력과 연관된 패턴을 학습한 스크리닝 모델
...
| 문진 모델 | Logistic Regression (KNHANES 기반, 진단 이력 기반 가능성 스크리닝) |
...
> 문진 스크리닝 모델(`survey_model/`)은 팀 내 데이터분석 파트가 KNHANES 데이터를 전처리·모델링한 결과를 넘겨받아 웹 서비스에 연동했습니다.
...
│
├── survey_model/                    # 문진형 아토피 가능성 스크리닝 파이프라인
...
│   │   ├── 3_분석_보고서.py        # 종합 보고서 (HTML 다운로드 포함)
...
**분석** 팀 자료를 다시 확인한 결과, calibration holdout set에서 Youden's J(민감도+특이도 균형)와 F1-optimal 두 기준으로 각각 threshold를 산출해둔 분석(`05_threshold_analysis.py`)이 별도로 있었습니다. 두 기준으로 각각 0.11, 0.17이 나왔습니다. 원래 스크립트는 이 두 값과 default 0.5까지 3개를 모두 test set에 적용해 평가하고 있어서, "test는 1회만 평가"라는 원래 설명과 실제 코드가 어긋나 있었습니다.

**개선** Youden's J 기준 threshold(0.11)를 운영 기준으로 채택해 앱에 반영하고, 스크립트도 calibration에서 F1-optimal(0.17)과 Youden's J(0.11)를 비교만 하고 test set에는 채택된 Youden's J 하나만 1회 적용하도록 정리했습니다. 그 결과 test 성능은 F1 0.40 / Precision 0.28 / Recall 0.67 수준입니다. 기존 3단계(낮음/보통/높음) 대신 기준치 이상/미만의 이진 판정으로 단순화했습니다 — 분석으로 확보한 운영점은 하나뿐이라, 등급을 더 나누면 그 경계에 대한 별도 통계적 근거가 없어지기 때문입니다.
...
- **모델 선정 기준 통일**: 이미지 모델 비교·체크포인트 저장 기준이 Accuracy였는데, Optuna 튜닝은 Macro F1을 최적화하고 있어 기준이 서로 달랐습니다. 클래스 불균형이 있는 문제라 Macro F1로 통일했습니다. 다만 아래 [주요 성능](#-주요-성능) 수치는 당시 Accuracy 기준으로 선정된 체크포인트로 실제 학습·평가했던 결과이며, 코드 수정 후 Macro F1 기준으로 재학습한 수치는 아닙니다. 재학습 시 소폭 달라질 수 있습니다.
...
- **문진 모델의 해석 범위**: 타깃이 아토피 진단 이력(과거)이라, 미래 발병 위험을 예측하는 모델이 아닙니다. 서비스 문구도 이에 맞춰 "위험도 예측"이 아닌 "아토피 가능성 스크리닝"으로 표현하고 있습니다.
