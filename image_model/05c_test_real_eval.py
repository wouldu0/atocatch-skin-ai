...
VARIANT = sys.argv[1] if len(sys.argv) > 1 else "dedup"
assert VARIANT in ("legacy", "dedup"), "variant는 legacy(기존) 또는 dedup(현재 production)"

# ── 설정 ──────────────────────────────────────────────────
PROJECT_ROOT = Path(__file__).resolve().parents[1]
MODEL_DIR_NAME = "mixed_v3_legacy_preleakfix" if VARIANT == "legacy" else "mixed_v3"
MODEL_PATH  = PROJECT_ROOT / "models" / MODEL_DIR_NAME / "efficientnetv2_s_tuned.pth"
TEST_DIR    = PROJECT_ROOT / "data" / "test_real"
SAVE_DIR    = PROJECT_ROOT / "models" / MODEL_DIR_NAME
os.makedirs(SAVE_DIR, exist_ok=True)
print(f"[VARIANT] {VARIANT} -> {MODEL_PATH}")
