# ============================================================
# 데이터 전처리 v2 — DermNet 아토피 클래스 정제 버전
#
# v1과 차이점:
#   DermNet "Atopic Dermatitis Photos" 폴더에서
#   파일명에 "atopic"/"Atopic" 포함된 이미지만 사용
#   (keratosis, ichthyosis 등 혼입 제거)
#
# Step 1. AI Hub 측면 이미지 → 300px JPEG 리사이즈
# Step 2. DermNet 이미지 필터링 + 300px JPEG 리사이즈
# Step 3. AI Hub + DermNet 합산 → skin_disease_mixed_v2.csv
# ============================================================
import os
import numpy as np
import pandas as pd
...
DERMNET_OUT_DIR = r"E:\skin\dermnet_300_v2"
AIHUB_OUT_CSV   = r"E:\skin\skin_disease_6class_side_300px.csv"
MIXED_OUT_CSV   = r"E:\skin\skin_disease_mixed_v2.csv"
...
# ════════════════════════════════════════════════════════════
# Step 2. DermNet 필터링 + 300px 리사이즈 (v2: 아토피 정제)
# ════════════════════════════════════════════════════════════
print("=" * 60)
print("Step 2. DermNet 이미지 필터링 및 리사이즈 (v2)")
print("  아토피: 파일명에 'atopic'/'Atopic' 포함된 것만 사용")
print("=" * 60)

# v2 변경: 아토피도 파일명 필터 적용 (keratosis, ichthyosis 제거)
DERMNET_MAP = [
    ("Atopic Dermatitis Photos",
     ["atopic", "Atopic"],   # v2: 아토피 파일명 필터 추가
     "아토피", 1),
...
def matches_prefix(fname, prefixes):
    if prefixes is None:
        return True
    return any(p in fname for p in prefixes)
...
print(f"\n완료!")
print(f"  DermNet v2 이미지: {DERMNET_OUT_DIR}")
print(f"  최종 CSV: {MIXED_OUT_CSV}")
print(f"\n다음 단계: train_mixed_v4.py 실행")
