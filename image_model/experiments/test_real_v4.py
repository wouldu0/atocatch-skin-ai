...
for row_i, cls in enumerate(CLASS_NAMES):
    ...
    for col_j in range(max_per_class):
        ...
        if col_j < len(cls_recs):
            ...
            title = f"{'OK' if r['correct'] else 'X'} {r['pred_label']}\n{r['probs'][r['pred_idx']]*100:.1f}%"
...
plt.suptitle(f"실제 이미지 테스트 결과 (v4)  |  전체 정확도: {correct_total/len(records)*100:.1f}%  ({correct_total}/{len(records)})",
...
plt.suptitle(f"클래스별 예측 확률 (v4)  |  전체 정확도: {correct_total/len(records)*100:.1f}%",
...
plt.title(f"Confusion Matrix (실제 이미지 테스트 v4)  |  Acc: {correct_total/len(records)*100:.1f}%",
          fontsize=12)
plt.tight_layout()
